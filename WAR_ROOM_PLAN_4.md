...
- [x] Config: add `check.mode: fail|warn` and `drift.ignore` defaults + merge/validation in `config.py`; document in `api-doc.yaml`
- [x] New `drift.py`: `detect_drift()`, `remove_ignored()` (dot-paths with `*`/`**`), item-level `iter_item_changes()`, `render_drift()` report
